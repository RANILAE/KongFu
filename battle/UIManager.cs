...
public class UIManager : MonoBehaviour
{
    ...
    // 移除 public Button loseRestartButton;

    // 新增：叠层不足提示面板 (通用，用于所有"条件不足"提示)
    [Header("Stack Insufficient Panel (通用)")]
    ...
    [Tooltip("玩家血量显示格式，使用 {0} 表示当前血量，{1} 表示最大血量")]
    public string playerHealthFormat = "{0}/{1}";
    [Tooltip("敌人血量显示格式，使用 {0} 表示当前血量，{1} 表示最大血量")]
    ...
    public void RestartBattle()
    {
        ...
        Debug.Log("Game resumed via UIManager.RestartBattle.");
        // ========== 新增结束 ==========
    ...
    // ========== 修改方法：显示敌人意图 ==========
    // 现在需要传递 EnemyAction，并尝试获取 EnemyType
    public void ShowEnemyIntent(EnemyManager.EnemyAction action)
    {
        // 尝试从 BattleSystem 获取 EnemyManager 以获取敌人类型
        EnemyManager enemyManager = null;
        EnemyManager.EnemyType enemyType = EnemyManager.EnemyType.Default; // 默认类型
        ...
        {
            // 调用 EnemyIntentDisplay 的新方法，传递 Action Type 和 Enemy Type
