...
public class BattleSystem : MonoBehaviour
{
    ...
    public void InitializeBattle()
    {
        ...
        if (playerManager != null)
        {
            ...
            {
                config.playerBaseHealth += PersistentBattleData.Instance.GetPlayerMaxHealthBonus();
            }
    ...
    public void RestartBattle()
    {
        ...
        {
            // 重置玩家配置到原始值
            BattleConfig originalConfig = config;
            if (PersistentBattleData.Instance != null)
            {
                // 重新计算玩家基础生命值
                int bonus = PersistentBattleData.Instance.GetPlayerMaxHealthBonus();
                originalConfig.playerBaseHealth = originalConfig.playerBaseHealth - bonus;
            }
            playerManager.Initialize(originalConfig, iconManager);
            // 重新应用生命值加成
            if (PersistentBattleData.Instance != null)
            {
                originalConfig.playerBaseHealth += PersistentBattleData.Instance.GetPlayerMaxHealthBonus();
            }
        }
    ...
    private void EndBattle(bool playerWins)
    {
        ...
        if (playerWins)
        {
            ...
            // 增加玩家生命上限
            PersistentBattleData.Instance.IncreasePlayerMaxHealth();
