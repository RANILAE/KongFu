...
public class YinYangTooltipManager : MonoBehaviour
{
    ...
    private string GetStateDescription(float diff, float yangPoints, float yinPoints)
    {
        ...
        if (absDiff < 1f)
        {
            return "Player recovers 5 HP (Continuous use won't trigger healing effect) CD: 2 turns";
        }
        else if (diff >= 1f && diff <= 2.5f)
        {
        ...
        {
            return "Next turn defense reduced by half (lasts until next turn ends), adds Counter Strike effect (only effective this turn). If counter strike occurs, damage is increased by 1.5x, and enemy's attack is reduced by layers*2 for 2 turns";
        }
        else if (absDiff > 7f && absDiff <= 10f)
        {
            return "Player's health reduces to 1 point, defense becomes 7*yin points (minimum 15), and gains Counter Strike effect for 3 turns";
        }
